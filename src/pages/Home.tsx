        ...
        <div style={{ width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          {participantsQuery.isLoading ? (
            ...
            <Select onValueChange={handleNameSelect}>
              <SelectTrigger style={{ width: '100%', maxWidth: 400, height: 56, fontSize: 18, borderRadius: 16, border: '2px solid #E6EDE8', background: '#FFFFFF', color: '#0B193D', padding: '0 20px', cursor: 'pointer' }}>
                <SelectValue placeholder="Kies je naam..." />
              </SelectTrigger>
              <SelectContent style={{ maxHeight: 300, overflowY: 'auto' }}>
                {participants.map((p) => (
                  <SelectItem key={p.id} value={p.name} style={{ fontSize: 16, padding: '12px 16px', cursor: 'pointer' }}>
                    {p.name}
                  </SelectItem>
                ))}
              </SelectContent>
