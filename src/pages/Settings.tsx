...
import { Mail, Lock, Users, Plus, X } from 'lucide-react';
...
function usePasswordGate() {
  ...
  return { unlocked, password, setPassword, error, setError, checkPassword };
}
...
export default function Settings() {
  ...
  const createParticipant = trpc.participant.create.useMutation({
    onSuccess: () => {
      utils.participant.listActive.invalidate();
  ...
  const handleAddParticipant = useCallback(() => {
    ...
    createParticipant.mutate({ name: newName.trim() });
    setNewName('');
  }, [newName, createParticipant]);
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {participantsQuery.isLoading ? (
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)' }}>Laden...</p>
            ) : (participantsQuery.data ?? []).length === 0 ? (
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)' }}>Nog geen deelnemers.</p>
            ) : (
